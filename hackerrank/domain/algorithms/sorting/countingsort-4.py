...
print the strings in their original order. Hence, your sorting algorithm
...
Details and a Twist
You will be given a list that contains both integers and strings. Can you
print the strings in order of their accompanying integers? If the integers
for two strings are equal, ensure that they are print in the order they
appeared in the original list.

The Twist - Your clients just called with an update. They don't want you to
print the first half of the original array. Instead, they want you to print
a dash for any element from the first half. So you can modify your counting
...
for i in range(100):
    if dict.has_key(i):
        for j in range(len(dict.get(i))):
            index = index_dict.get(i)[j]
            s = dict.get(i)[j]
            if index < mid:
                final_list.append("-")
            else:
