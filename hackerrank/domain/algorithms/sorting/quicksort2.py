...
one element at a time, but instead must deal with whole sub-arrays, with a
...
Task
The method quickSort takes in a parameter, ar, an unsorted array. Use the
...
def partition(ar):
    ...
    for i in range(1, n):
        if p > ar[i]:
            ...
            greater.append(ar[i])
    ar = partition(lesser) + [p] + partition(greater)
