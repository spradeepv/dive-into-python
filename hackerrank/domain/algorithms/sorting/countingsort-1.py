...
non-comparison sorting algorithm. This will make it possible to sort lists
even in linear time. These challenges will cover Counting Sort, a fast way
to sort lists where the elements have a small number of possible values,
such as integers within a certain range. We will start with an easy task -
counting.
...
The output states that 0 appears 0 times, 1 appears 2 times, 2 appears 0
times, and so on in the given input array.
"""
def count(ar):
    l = []
    for i in range(100):
