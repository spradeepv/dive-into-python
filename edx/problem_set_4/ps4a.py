...
def updateHand(hand, word):
    ...
    updated_hand = {}
    for x in hand:
        updated_hand[x] = hand.get(x,0) - word.count(x)
    return updated_hand
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for c in word:
            if c not in temp_hand.keys() or temp_hand.get(c) == 0:
                is_valid_word = False
                break
            else:
                temp_hand[c] = temp_hand.get(c) - 1
    else:
        is_valid_word = False
...
def playGame(wordList):
    ...
    hand = {}
    while user_input != 'e':
        user_input = raw_input("Enter n to deal a new hand, r to replay the "
