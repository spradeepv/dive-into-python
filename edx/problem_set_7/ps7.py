...
class NewsStory(object):
    ...
    def __init__(self, guid, title_text, subject_text, summary_text, link):
        self.guid = guid
...
class PhraseTrigger(Trigger):
    ...
    def evaluate(self, story):
        return story.getSubject().find(self.phrase) != -1 \
               or story.getTitle().find(self.phrase) != -1 \
               or story.getSummary().find(self.phrase) != -1
    ...
    for story in stories:
        ...
        for trigger in triggerlist:
            if trigger.evaluate(story):
                triggered = True
                break
        if triggered and story not in story_list:
